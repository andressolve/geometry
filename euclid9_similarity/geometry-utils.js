/**
 * Geometry Utilities
 * Shared drawing functions for Euclidean Geometry lessons
 */
...
const Colors = {
    ...
    pair4: { fill: 'rgba(139, 92, 246, 0.3)', stroke: 'rgb(139, 92, 246)' },   // Purple
    warning: { fill: 'rgba(245, 158, 11, 0.3)', stroke: 'rgb(245, 158, 11)' }  // Orange/Warning
};
...
const GeometryUtils = {
    ...
    /**
     * Get point on circle given center, radius, and angle (in radians)
     */
    pointOnCircle(center, radius, angleRad) {
        return {
    ...
    /**
     * Calculate angle at vertex given three points (vertex, p1, p2)
     * Returns angle in degrees
     */
    angleAtVertex(vertex, p1, p2) {
        const angle1 = Math.atan2(p1.y - vertex.y, p1.x - vertex.x);
        const angle2 = Math.atan2(p2.y - vertex.y, p2.x - vertex.x);
        let diff = Math.abs(angle2 - angle1);
        if (diff > Math.PI) diff = 2 * Math.PI - diff;
        return this.toDegrees(diff);
    },
    ...
    /**
     * Draw an extended line (extends beyond two points)
     */
    drawLine(ctx, p1, p2, canvasWidth, canvasHeight, color = '#333333', width = 2) {
        const dx = p2.x - p1.x;
        const dy = p2.y - p1.y;
        ...
        const start = { x: p1.x - dirX * extension, y: p1.y - dirY * extension };
        const end = { x: p1.x + dirX * extension, y: p1.y + dirY * extension };
     ...
     * Draw a circle
     */
    drawCircle(ctx, center, radius, color = '#333333', width = 2) {
        ctx.beginPath();
     ...
     */
    drawArc(ctx, center, radius, startAngle, endAngle, color = '#333333', width = 3) {
        ctx.beginPath();
     ...
     */
    drawRightAngle(ctx, vertex, p1, p2, size = 12, color = '#333333') {
        // Get angles to the two points
        const angle1 = Math.atan2(p1.y - vertex.y, p1.x - vertex.x);
        const angle2 = Math.atan2(p2.y - vertex.y, p2.x - vertex.x);

        // Calculate corner points of the square
        const corner1X = vertex.x + size * Math.cos(angle1);
        const corner1Y = vertex.y + size * Math.sin(angle1);

        const corner2X = vertex.x + size * Math.cos(angle2);
        const corner2Y = vertex.y + size * Math.sin(angle2);

        const cornerMidX = vertex.x + size * Math.cos(angle1) + size * Math.cos(angle2);
        const cornerMidY = vertex.y + size * Math.sin(angle1) + size * Math.sin(angle2);

        // Draw the square
        ctx.beginPath();
        ctx.moveTo(corner1X, corner1Y);
        ctx.lineTo(cornerMidX, cornerMidY);
    ...
    /**
     * Draw angle label with degree symbol
     */
    drawAngleLabel(ctx, vertex, p1, p2, label, radius = 40, color = '#333333') {
        const angle1 = Math.atan2(p1.y - vertex.y, p1.x - vertex.x);
        const angle2 = Math.atan2(p2.y - vertex.y, p2.x - vertex.x);
        ...
        const labelX = vertex.x + radius * Math.cos(midAngle);
        const labelY = vertex.y + radius * Math.sin(midAngle);
