...
const Page3 = {
    ...
    init() {
        ...
        if (!this._initialized) {
            ...
            const stepBtn = document.getElementById('build3-step');
    ...
    step() {
        ...
        const btn = document.getElementById('build3-step');
        const counter = document.getElementById('build3-counter');
        if (counter) counter.textContent = `${this.currentStep} of ${this.maxSteps}`;
    ...
    draw(lineT = 0) {
        ...
        if (lineT > 0) {
            GridUtils.drawLinePartial(this.buildCtx, this.buildCs, x => this.slope * x, lineT, {
        ...
        for (let i = 0; i < this.points.length - 1; i++) {
            const p = this.points[i];
            GridUtils.drawStepTriangle(this.buildCtx, this.buildCs, p.x, p.y, 1, this.slope, {
